...
export function MetricEditor({spec,input,onSave,onClose}:{spec:MetricSpec;input?:MetricInput;onSave:(input?:MetricInput)=>void;onClose:()=>void}){
 ...
 const [date,setDate]=useState(input?.date??''),[period,setPeriod]=useState(input?.period??''),[url,setUrl]=useState(input?.url??''),[note,setNote]=useState(input?.note??''),[currency,setCurrency]=useState<'USD'|'SGD'>(input?.currency??'SGD'),[error,setError]=useState('');
 const draft:MetricInput={values:values.map(x=>x.trim()===''?NaN:Number(x)),date,period,url,note,currency,origin:'user'};
